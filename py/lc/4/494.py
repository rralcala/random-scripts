...
class Solution:
    ...
    def _findTargetSumWays(
        self, nums: List[int], s: int, depth, sum: int, i: int
    ) -> int:
        if depth == 0:
            # print(sign)
            if s == sum:
        ...
        if sum in memoi:
            ...
            memoi[sum] = self._findTargetSumWays(
                nums, s, depth, sum + nums[i], i + 1
            ) + self._findTargetSumWays(nums, s, depth, sum - nums[i], i + 1)
            return memoi[sum]
