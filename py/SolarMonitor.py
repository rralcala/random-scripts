...
DELAY_BETWEEN_REQUESTS = (
    3  # Number of seconds to wait in between requests to the charge controller.
)
...
def printDataText(response):
    ...
    print(
        "Battery Voltage:\t\t" + str(round(float(response.registers[1] * 0.1), 1)) + "V"
    )
    print(
        "Battery Charge Current:\t\t"
        + str(round(float(response.registers[2] * 0.01), 2))
        + "A"
    )
    print(
        "Controller Temperature:\t\t"
        + str(getRealTemp(int(hex(response.registers[3])[2:-2], 16)))
        + "*C"
    )
    print(
        "Battery Temperature:\t\t"
        + str(getRealTemp(int(hex(response.registers[3])[-2:], 16)))
        + "*C"
    )
    print(
        "Load Voltage:\t\t\t" + str(round(float(response.registers[4] * 0.1), 1)) + " V"
    )
    print(
        "Load Current:\t\t\t"
        + str(round(float(response.registers[5] * 0.01), 2))
        + " A"
    )
    print("Load Power:\t\t\t" + str(response.registers[6]) + " Watts")
    print("Load Enabled:\t\t\t" + str(response.registers[32] > 6))
    print(
        "Panel Volts:\t\t\t" + str(round(float(response.registers[7] * 0.1), 1)) + "V"
    )
    print(
        "Panel Amps:\t\t\t" + str(round(float(response.registers[8] * 0.01), 2)) + "A"
    )
    ...
    print(
        ...
        + str(round(float(response.registers[11] * 0.1), 1))
        + "V"
    )
    print(
        "Battery Maximum Voltage:\t"
        + str(round(float(response.registers[12] * 0.1), 1))
        + "V"
    )
    print(
        "Maximum Charge Current:\t\t"
        + str(round(float(response.registers[13] * 0.01), 2))
        + "A"
    )
    print("Maximum Charge Power:\t\t" + str(response.registers[15]) + "W")
    print(
        "Maximum Load Discharge Current:\t"
        + str(float(response.registers[14]) * 0.01)
        + "A"
    )
    ...
    print(
        "Charge Power:\t\t\t"
        + str(round(float(response.registers[19] * 0.001), 3))
        + "KWh"
    )
    print("Load Amp Hours:\t\t\t" + str(response.registers[18]) + "Ah")
    print(
        "Load Power:\t\t\t"
        + str(round(float(response.registers[20] * 0.001), 3))
        + "KWh"
    )
    ...
    print(
        ...
        + str(
            round(
                float(
                    (response.registers[24] * 65536 + response.registers[25]) * 0.001
                ),
                3,
            )
        )
    ...
    print(
        ...
        + str(
            round(
                float(
                    (response.registers[28] * 65536 + response.registers[29]) * 0.001
                ),
                3,
            )
        )
    ...
    print(
        ...
        + str(
            round(
                float(
                    (response.registers[26] * 65536 + response.registers[27]) * 0.001
                ),
                3,
            )
        )
    ...
    print(
        ...
        + str(
            round(
                float(
                    (response.registers[30] * 65536 + response.registers[31]) * 0.001
                ),
                3,
            )
        )
    ...
    if error:
        ...
        nested_dict = {
            ...
            "charging": {
                ...
                "totalAmpHours": round(
                    float(
                        (response.registers[24] * 65536 + response.registers[25])
                        * 0.001
                    ),
                    3,
                ),
                ...
                "totalPower": round(
                    float(
                        (response.registers[28] * 65536 + response.registers[29])
                        * 0.001
                    ),
                    3,
                ),
            ...
            "load": {
                ...
                "totalAmpHours": round(
                    float(
                        (response.registers[26] * 65536 + response.registers[27])
                        * 0.001
                    ),
                    3,
                ),
                ...
                "totalPower": str(
                    round(
                        float(
                            (response.registers[30] * 65536 + response.registers[31])
                            * 0.001
                        ),
                        3,
                    )
